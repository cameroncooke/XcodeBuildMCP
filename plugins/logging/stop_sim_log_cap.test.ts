...
describe('stop_sim_log_cap plugin', () => {
  ...
  describe('plugin structure', () => {
    it('should have correct plugin structure', () => {
      ...
      expect(stopSimLogCap.description).toBe('Stops an active simulator log capture session and returns the captured logs.');
  ...
  describe('handler functionality', () => {
    ...
    it('should include log content in successful response', async () => {
      ...
      expect(result.content[0].text).toContain('Log capture session test-session-id stopped successfully');
    });
  });
});