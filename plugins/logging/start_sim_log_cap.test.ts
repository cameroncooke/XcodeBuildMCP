...
describe('start_sim_log_cap plugin', () => {
  ...
  describe('Plugin Structure', () => {
    ...
    it('should have correct description', () => {
      expect(plugin.description).toBe('Starts capturing logs from a specified simulator. Returns a session ID. By default, captures only structured logs.');
    });
  ...
  describe('Handler Functionality', () => {
    ...
    it('should return error when log capture fails', async () => {
      ...
      expect(result.content[0].text).toContain('Error starting log capture: Failed to start log capture');
    });
    ...
    it('should return success with session ID when log capture starts successfully', async () => {
      ...
      expect(result.content[0].text).toContain('Log capture started successfully. Session ID: test-session-123');
    ...
    it('should indicate console capture when captureConsole is true', async () => {
      ...
      expect(result.content[0].text).toContain('Note: Your app was relaunched to capture console output.');
    });
    ...
    it('should include next steps in success message', async () => {
      ...
      expect(result.content[0].text).toContain("Use 'stop_sim_log_cap' with session ID 'test-session-123'");
    });
  });
});