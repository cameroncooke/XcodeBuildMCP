...
import { loadWorkflowGroups } from '../../src/utils/index.js';
import { enableWorkflows } from '../../src/utils/index.js';

export default {
  name: 'discover_tools',
  description: 'Analyzes a natural language task description to enable a relevant set of Xcode and Apple development tools for the current session.',
  schema: {
    task_description: z
      .string()
      .describe(
...
Each workflow contains ALL tools needed for its complete development workflow - no need to combine workflows.`;
      ...
      try {
        ...
        const validWorkflows = selectedWorkflows.filter((workflow) => workflowNames.includes(workflow));
      ...
      await enableWorkflows(server, selectedWorkflows, workflowGroups);
