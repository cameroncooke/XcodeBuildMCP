 ...
 */
function getBuildSettings(projectPath: string, projectType: 'workspace' | 'project', scheme: string): any {
  try {
    const flag = projectType === 'workspace' ? '-workspace' : '-project';
    const output = execSync(`xcodebuild ${flag} "${projectPath}" -scheme "${scheme}" -showBuildSettings`, {
      encoding: 'utf8',
      timeout: 15000
    });
...
function findLatestBuildLog(): string | null {
  try {
    const derivedDataPath = execSync('xcodebuild -showBuildSettings | grep "BUILD_DIR" | head -1 | cut -d "=" -f2 | xargs dirname', {
      encoding: 'utf8',
      timeout: 5000
    }).trim();
    ...
    const logsPath = join(derivedDataPath, 'Logs', 'Build');
    ...
    const logFiles = readdirSync(logsPath)
      ...
      .map(file => ({
        ...
        mtime: statSync(join(logsPath, file)).mtime
      }))
        ...
        scheme: async (value: string) => {
          ...
          try {
            ...
            return schemes.filter(scheme => 
              scheme.toLowerCase().includes(value.toLowerCase())
            );
          } catch (error) {
            return [];
          }
