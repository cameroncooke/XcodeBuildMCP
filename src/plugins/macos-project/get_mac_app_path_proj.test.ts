...
describe('get_mac_app_path_proj', () => {
  ...
  describe('Handler Behavior (Complete Literal Returns)', () => {
    ...
    it('should handle projectPath validation failure', async () => {
      // Mock validation failure
      const mockErrorResponse = {
        content: [
          {
            type: 'text',
            text: 'Error: projectPath is required',
          },
        ],
        isError: true,
      };
      ...
      expect(mockValidateRequiredParam).toHaveBeenCalledWith('projectPath', undefined);
      expect(result).toEqual({
        content: [
          {
            type: 'text',
    ...
    it('should handle scheme validation failure', async () => {
      // Mock successful projectPath validation but failed scheme validation
