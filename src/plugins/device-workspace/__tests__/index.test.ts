/**
 * Tests for device-workspace workflow metadata
 */
import { describe, it, expect } from 'vitest';
import { workflow } from '../index.ts';

describe('device-workspace workflow metadata', () => {
  describe('Workflow Structure', () => {
    ...
    it('should have correct workflow name', () => {
      expect(workflow.name).toBe('iOS Device Workspace Development');
    });

    it('should have correct description', () => {
      expect(workflow.description).toBe(
        'Complete iOS development workflow for .xcworkspace files targeting physical devices (iPhone, iPad, Apple Watch, Apple TV, Apple Vision Pro). Build, test, deploy, and debug on real hardware.',
      );
    });
