...
// Extracted components for describe_ui tool
export const describeUIToolName = 'describe_ui';
export const describeUIToolDescription =
...
export const describeUIToolSchema = {
  simulatorUuid: z.string().uuid('Invalid Simulator UUID format'),
};

// Extracted components for touch tool
export const touchToolName = 'touch';
export const touchToolDescription =
...
export const describeUIToolHandler = async (params: {
...
export const touchToolHandler = async (params: {
  simulatorUuid: string;
  x: number;
  y: number;
  down?: boolean;
...
// Extracted components for tap tool
export const tapToolName = 'tap';
export const tapToolDescription =
...
export const tapToolHandler = async (params: {
  simulatorUuid: string;
  x: number;
  y: number;
  preDelay?: number;
...
// Extracted components for long_press tool
export const longPressToolName = 'long_press';
export const longPressToolDescription =
...
export const longPressToolHandler = async (params: {
  simulatorUuid: string;
  x: number;
  y: number;
  duration: number;
...
// Extracted components for swipe tool
export const swipeToolName = 'swipe';
export const swipeToolDescription =
...
export const swipeToolHandler = async (params: {
  simulatorUuid: string;
  x1: number;
  y1: number;
  x2: number;
...
// Extracted components for type_text tool
export const typeTextToolName = 'type_text';
export const typeTextToolDescription =
...
export const typeTextToolHandler = async (params: {
  simulatorUuid: string;
  text: string;
}): Promise<ToolResponse> => {
...
// Extracted components for key_press tool
export const keyPressToolName = 'key_press';
export const keyPressToolDescription =
...
export const keyPressToolHandler = async (params: {
  simulatorUuid: string;
  keyCode: number;
  duration?: number;
}): Promise<ToolResponse> => {
...
// Extracted components for button tool
export const buttonToolName = 'button';
export const buttonToolDescription =
  'Press hardware button on iOS simulator. Supported buttons: apple-pay, home, lock, side-button, siri';
export const buttonToolSchema = {
  simulatorUuid: z.string().uuid('Invalid Simulator UUID format'),
  buttonType: z.enum(['apple-pay', 'home', 'lock', 'side-button', 'siri']),
  duration: z.number().min(0, 'Duration must be non-negative').optional(),
};
export const buttonToolHandler = async (params: {
  simulatorUuid: string;
...
// Extracted components for key_sequence tool
export const keySequenceToolName = 'key_sequence';
export const keySequenceToolDescription =
  'Press key sequence using HID keycodes on iOS simulator with configurable delay';
export const keySequenceToolSchema = {
  simulatorUuid: z.string().uuid('Invalid Simulator UUID format'),
  keyCodes: z.array(z.number().int().min(0).max(255)).min(1, 'At least one keycode required'),
  delay: z.number().min(0, 'Delay must be non-negative').optional(),
};
export const keySequenceToolHandler = async (params: {
  simulatorUuid: string;
  keyCodes: number[];
  delay?: number;
}): Promise<ToolResponse> => {
...
// Extracted components for gesture tool
export const gestureToolName = 'gesture';
export const gestureToolDescription =
...
export const gestureToolSchema = {
  simulatorUuid: z.string().uuid('Invalid Simulator UUID format'),
  preset: z.enum([
    'scroll-up',
    'scroll-down',
...
export const gestureToolHandler = async (params: {
  simulatorUuid: string;
  preset: string;
  screenWidth?: number;
  screenHeight?: number;
...
export function registerAxeTools(server: McpServer): void {
  ...
  // 2. tap
  server.tool(tapToolName, tapToolDescription, tapToolSchema, tapToolHandler);

  // 3. long_press
  server.tool(
    longPressToolName,
    longPressToolDescription,
    longPressToolSchema,
    longPressToolHandler,
  );

  // 4. swipe
  server.tool(swipeToolName, swipeToolDescription, swipeToolSchema, swipeToolHandler);

  // 5. type_text
  server.tool(typeTextToolName, typeTextToolDescription, typeTextToolSchema, typeTextToolHandler);

  // 6. key_press
  server.tool(keyPressToolName, keyPressToolDescription, keyPressToolSchema, keyPressToolHandler);

  // 7. button
  server.tool(buttonToolName, buttonToolDescription, buttonToolSchema, buttonToolHandler);
  ...
  // 9. touch
  server.tool(touchToolName, touchToolDescription, touchToolSchema, touchToolHandler);

  // 10. gesture
  server.tool(gestureToolName, gestureToolDescription, gestureToolSchema, gestureToolHandler);
}
