...
describe('detectPlatformFromScheme', () => {
  ...
  it('returns null platform for non-simulator SDKROOT values', async () => {
    ...
    expect(result.supportedPlatforms).toEqual(['macosx']);
  });

  it('returns null platform for device SDKROOT values', async () => {
    const executor = createMockExecutor({
      success: true,
    ...
    expect(result.platform).toBeNull();
    expect(result.sdkroot).toBe('iphoneos');
    expect(result.supportedPlatforms).toEqual(['iphoneos']);
  });
...
`,
  ...
  it('returns null platform for device-only SUPPORTED_PLATFORMS', async () => {
    const executor = createMockExecutor({
      success: true,
      output: 'SUPPORTED_PLATFORMS = iphoneos watchos appletvos',
    });
    ...
    expect(result.platform).toBeNull();
    expect(result.sdkroot).toBeNull();
    expect(result.supportedPlatforms).toEqual(['iphoneos', 'watchos', 'appletvos']);
  });
  ...
  it('returns error when neither projectPath nor workspacePath is provided', async () => {
    const executor = createMockExecutor({
      success: true,
      output: 'SDKROOT = iphonesimulator',
    });
